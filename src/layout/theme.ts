...
const PALE_BRANCH_LUMINANCE = 0.72;
const PALE_CONNECTOR_DARKEN = 0.38;

/** Readable connector stroke for every branch, including pale palette accents. */
export function connectorStrokeColor(
   branchIndex: number,
   themeId: string = DEFAULT_MAP_THEME_ID,
): string {
   const { color } = getBranchTheme(branchIndex, themeId);
   if (relativeLuminance(color) <= PALE_BRANCH_LUMINANCE) return color;
   return mixWithBlack(color, PALE_CONNECTOR_DARKEN);
}
